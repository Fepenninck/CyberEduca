...
import { BookOpen, Camera, Check, ChevronLeft, ClipboardCheck, Clock3, Home, LockKeyhole, MonitorPlay, Pencil, Save, ShieldCheck, UserRound, X } from 'lucide-react'
...
export default function PerfilPage() {
  const [profile, setProfile] = useState<Profile>({ nome: 'Felipe Penninck', email: 'felipe@cybereduca.com' })
  ...
  function applyCrop() {
    ...
    image.onload = () => {
      ...
      setProfile((current) => ({ ...current, foto: canvas.toDataURL('image/jpeg', .9) }))
      setCropSource('')
    }
  ...
  return <main className="app-shell dashboard-page profile-page">
    <header className="dashboard-header"><div className="dashboard-header-inner">
      <Link href="/dashboard" className="dashboard-brand" aria-label="Voltar ao dashboard"><img src="/cybereduca-logo.png" alt="CyberEduca+" /></Link>
      <nav className="dashboard-nav" aria-label="Navegação da área do aluno"><Link href="/dashboard"><Home size={16} /> Início</Link><Link className="is-active" href="/perfil"><UserRound size={16} /> Perfil</Link><Link href="/trilhas"><BookOpen size={16} /> Minhas aulas</Link><Link href="/em-breve"><MonitorPlay size={16} /> Simulações</Link><Link href="/em-breve"><ClipboardCheck size={16} /> Avaliações</Link></nav>
    </div></header>
    <section className="profile-wrap">
      ...
      <header className="profile-hero">
        ...
        <div className="profile-identity"><span className="profile-kicker">Perfil do aluno</span><div><h1>{profile.nome}</h1><span className="profile-handle">@{profile.nome.toLowerCase().replace(/\s+/g, '')}</span></div><p>Aluno CyberEduca+</p></div>
        <div className="profile-hero-actions"><button type="button" onClick={() => setActiveTab('settings')}>Editar</button><button type="button" onClick={() => fileInput.current?.click()}>Alterar foto</button></div>
      </header>

      <div className="profile-tabs"><button className={activeTab === 'overview' ? 'is-active' : ''} type="button" onClick={() => setActiveTab('overview')}>Visão geral</button><button className={activeTab === 'settings' ? 'is-active' : ''} type="button" onClick={() => setActiveTab('settings')}>Configurações</button></div>
      ...
      {activeTab === 'settings' && (
        <div className="profile-settings-backdrop" role="dialog" aria-modal="true" aria-labelledby="profile-settings-title">
          <article className="profile-card profile-settings profile-settings-modal">
            ...
            <div className="profile-settings-heading"><div><h2 id="profile-settings-title">Configurações do perfil</h2><p>Você controla suas informações e pode atualizá-las quando quiser.</p></div><button className="profile-photo-button" type="button" onClick={() => fileInput.current?.click()}><Camera size={16} /> Alterar foto</button></div>
            ...
            <div className="profile-modal-extra"><label><input type="checkbox" /> Manter meu nome visível no perfil.</label><small>Você pode mudar essa preferência quando quiser.</small><div><label>Apresentação <span>0/60</span><input placeholder="Conte brevemente o que está aprendendo" maxLength={60} /></label><small>Essa informação ficará disponível quando seu perfil público estiver pronto.</small></div></div>
            <div className="profile-privacy"><LockKeyhole size={18} /><div><strong>Privacidade e proteção de dados</strong><p>Seus dados de conta são usados somente para identificar seu acesso e acompanhar o aprendizado.</p></div></div>
